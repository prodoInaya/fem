...
FEM<dim>::FEM ()
:
fe (FE_Q<dim>(1), 1),  // (1) для линейных базисных функций
...
double FEM<dim>::basis_function(unsigned int node, double xi_1, double xi_2, double xi_3){
  ...
  switch(node) {
    case 0: value = (1 - xi_1) * (1 - xi_2) * (1 - xi_3); break;
    case 1: value = (1 + xi_1) * (1 - xi_2) * (1 - xi_3); break;
    case 2: value = (1 - xi_1) * (1 + xi_2) * (1 - xi_3); break;
    case 3: value = (1 + xi_1) * (1 + xi_2) * (1 - xi_3); break;
    case 4: value = (1 - xi_1) * (1 - xi_2) * (1 + xi_3); break;
    case 5: value = (1 + xi_1) * (1 - xi_2) * (1 + xi_3); break;
    case 6: value = (1 - xi_1) * (1 + xi_2) * (1 + xi_3); break;
    case 7: value = (1 + xi_1) * (1 + xi_2) * (1 + xi_3); break;
  }
...
std::vector<double> FEM<dim>::basis_gradient(unsigned int node,
  ...
  switch(node) {
    case 0:
      values[0] = -(1 - xi_2) * (1 - xi_3);
      values[1] = -(1 - xi_1) * (1 - xi_3);
      values[2] = -(1 - xi_1) * (1 - xi_2);
      break;
    case 1:
      values[0] = (1 - xi_2) * (1 - xi_3);
      values[1] = -(1 + xi_1) * (1 - xi_3);
...
void FEM<dim>::define_boundary_conds(){
  ...
  /*Note: this will be very similiar to the define_boundary_conds function
    ...
    to check if the node is on the boundary with a Dirichlet condition. If it
    is, then add the node number and the specified value (temperature in this
    problem) to the boundary values map, something like this:

    boundary_values[globalNodeIndex] = dirichletTemperatureValue

    Note that "nodeLocation" is now a Table instead of just a vector. The row
    index is the global node number; the column index refers to the x, y, or z
    component (0, 1, or 2 for 3D). e.g. nodeLocation[7][2] is the z coordinate
    of global node 7*/

  const unsigned int totalNodes = dof_handler.n_dofs(); //Total number of nodes

  //EDIT - Define the Dirichlet boundary conditions.
  // WARNING: this should be done in setup_system
              ...
              for(unsigned int i=0;i<dim;i++){
                for(unsigned int j=0;j<dim;j++){
                  for(unsigned int I=0;I<dim;I++){
                    for(unsigned int J=0;J<dim;J++){
                      //EDIT - Define Klocal.
                      // Notice the Jacobian is evaluated at the quad points
                      Klocal[A][B] += basis_gradient(A, 
                                                    quad_points[q1],
                                                    quad_points[q2], 
                                                    quad_points[q3])[I]
                                      * invJacob[I][i]
