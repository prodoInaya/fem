...
double FEM<dim>::basis_function(unsigned int node, double xi_1, double xi_2){
  ...
  switch(node) {
    case 0: value = (1 - xi_1) * (1 - xi_2); break;
    case 1: value = (1 + xi_1) * (1 - xi_2); break;
    case 2: value = (1 - xi_1) * (1 + xi_2); break;
    case 3: value = (1 + xi_1) * (1 + xi_2); break;
  }
...
void FEM<dim>::generate_mesh(std::vector<unsigned int> numberOfElements){
  ...
  // Store the limits in class variables
  double x_min = 0., //EDIT
    x_max = 0.03, //EDIT
    y_min = 0., //EDIT
    y_max = 0.08; //EDIT
  ...
  for(unsigned int globalNode=0; globalNode<totalNodes; globalNode++)
{
  ...
  {
   boundary_values[globalNode] = 310.0*(1.0 + 8.0*nodeLocation[globalNode][0]*nodeLocation[globalNode][0]);
  }
}
}
...
void FEM<dim>::assemble_system(){
  ...
  for (;elem != endc; ++elem) {
    ...
    // Flocal is provided here in cases ther eis any forcing function
